...
(0, node_test_1.default)('returns the upstream current round payload', async () => {
    const payload = { round: 12, status: 'active', prize: 777, entries: 88, endTime: 1741526200000 };
    let requestedUrl = '';
    let requestedInit;
    ...
    strict_1.default.equal(requestedUrl, 'https://api.rore.supply/api/rounds/current');
...
(0, node_test_1.default)('returns a 500 response when the upstream current round API fails', async () => {
    console.error = () => { };
    mockFetch(async () => new Response(null, { status: 503 }));
    const response = await (0, route_1.GET)();
    strict_1.default.equal(response.status, 500);
    strict_1.default.deepEqual(await response.json(), { error: 'Failed to fetch current round data' });
    strict_1.default.equal(response.headers.get('Access-Control-Allow-Origin'), '*');
});
(0, node_test_1.default)('returns a 500 response when the current round fetch throws', async () => {
    console.error = () => { };
    mockFetch(async () => {
